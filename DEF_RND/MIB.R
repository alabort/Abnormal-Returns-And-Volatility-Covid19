...
#select the date of the desired RND (it is necessary to change the date also one more time later in row 69)
Selected_Date <- "09/04/2020"
Selected_Date2 <- "09/04/20"

#import the prices of the underlying and filter the price of the wanted day
Price <-  read.csv("MIBS0.csv", header = TRUE, sep = ";", dec=".")
Exact2<- Price %>% filter(Price$ï..Date == Selected_Date) 
...
Exact <- Rates %>% filter(Rates$ï.. == Selected_Date)
...
Main_table <- read.csv("MIB.csv", header = TRUE, sep = ",", dec=".")
#remember to change the date in the following line, the format should be X.dd.mm.yyyy
Date_table <- data.frame(Main_table$X09.04.2020, Main_table$Strike, Main_table$Expiry.Date, Main_table$X.NAME.)
Date_table2 <- Date_table[complete.cases(Date_table), ]
Date_table_put <- Date_table2 %>% filter(Main_table.X.NAME.== "P-EU")
Date_table_call <- Date_table2 %>% filter(Main_table.X.NAME.== "C-EU")
...
#the yield of the FTSE MIB has been downloaded from https://www.borsaitaliana.it/borsa/indici/indici-di-dividendo/dettaglio.html?indexCode=MIBDIV&lang=it
y1 =0.0239
y2 =0.0239
...
variable06 <- data.frame(Column1 = c(x.mean, x.med, x.mode, mln.sdlog.1, mln.sdlog.2, IQR, mln.alpha.1, 1- mln.alpha.1, x.mean2, x.med2, x.mode2, mln2.sdlog.1, mln.sdlog.2, IQR2, mln2.alpha.1, 1- mln2.alpha.1))
