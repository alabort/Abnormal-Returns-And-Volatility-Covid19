...
Selected_Date <- "20/04/2020"
Selected_Date2 <- "20/04/20"

#import the prices of the underlying and filter the price of the wanted day
Price <-  read.csv("AAS0.csv", header = TRUE, sep = ";", dec=".")
...
te2 = as.numeric((as.Date("2021-01-15")-as.Date(Selected_Date, format = "%d/%m/%y"))/365) 

#import the LIBOR rates for dollar and filter the rates for the selected date 
#the 1M, 3M, 6M and 12M rates have been downloaded from FRED
Rates <- read.csv("USRATE.csv", header = TRUE, sep = ";", dec=".")
...
r2 <- as.numeric(as.vector(termStruct.spline.interpolate["2021-01-15", 1]))
...
Main_table <- read.csv("AMEX_TOT.csv", header = TRUE, sep = ",", dec=".")
#remember to change the date in the following line, the format should be X.dd.mm.yyyy
Date_table <- data.frame(Main_table$X20.04.2020, Main_table$Strike, Main_table$Expiry.Date, Main_table$X.NAME.)
...
Put_mat2 <- Date_table_put %>% filter(Expiry.date == "15/01/2021")
Call_mat2 <- Date_table_call %>% filter(Expiry.date == "15/01/2021")
...
xls <-c(Call_mat1$Strike,Put_mat1$Strike)
...
legend("topright", legend=c("21-08-2020", "15-01-2021"), col=c("black","blue","red"), 
