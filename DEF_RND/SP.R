...
Selected_Date <- "22/04/2020"
Selected_Date2 <- "22/04/20"

#import the prices of the underlying and filter the price of the wanted day
Price <-  read.csv("SPS0.csv", header = TRUE, sep = ";", dec=".")
...
#import the LIBOR rates for US dollar and filter the rates for the selected date 
...
Main_table <- read.csv("SP.csv", header = TRUE, sep = ",", dec=".")
#remember to change the date in the following line, the format should be X.dd.mm.yyyy
Date_table <- data.frame(Main_table$X22.04.2020, Main_table$Strike, Main_table$Expiry.Date, Main_table$X.NAME.)
...
#the yield of S&P500 has been downloaded from https://www.quandl.com/data/MULTPL/SP500_DIV_YIELD_MONTH-S-P-500-Dividend-Yield-by-Month
y1 =0.04
y2 =0.04
