...
#select the date of the desired RND (it is necessary to change the date also one more time later in row 66)
Selected_Date <- "11/03/2020"
Selected_Date2 <- "11/03/20"

#import the prices of the underlying and filter the price of the wanted day
Price <-  read.csv("CES0.csv", header = TRUE, sep = ";", dec=".")
Exact2<- Price %>% filter(Price$Timestamp == Selected_Date) 
s0 = as.numeric(Exact2$Trade.Close)

#computation of the time to maturity for for one maturity
#other maturities are too illiquid
te1 = as.numeric((as.Date("2020-08-21")-as.Date(Selected_Date, format = "%d/%m/%y"))/365) 

#import the LIBOR rates for Chinese yen and filter the rates for the selected date 
#the 1M, 3M, 6M and 12M rates have been downloaded from the official SHIBOR website
Rates <- read.csv("SHIBOR.csv", header = TRUE, sep = ";", dec=".")
...
marketRates <- c(0.0, Exact$X1M, Exact$X3M, Exact$X6M, Exact$X1Y) * 0.01
...
#spline interpolation of rates
r1 <- as.numeric(as.vector(termStruct.spline.interpolate["2020-08-21", 1]))
...
Main_table <- read.csv("CE.csv", header = TRUE, sep = ",", dec=".")
#remember to change the date in the following line, the format should be X.dd.mm.yyyyMain_table <- read.csv("CE.csv", header = TRUE, sep = ",", dec=".")
Date_table <- data.frame(Main_table$X11.03.2020.1, Main_table$Strike, Main_table$Expiry.Date, Main_table$X.NAME.)
...
Put_mat1 <- Date_table_put %>% filter(Expiry.date == "21/08/2020")
Call_mat1 <- Date_table_call %>% filter(Expiry.date == "21/08/2020")
Put_mat2 <- Date_table_put %>% filter(Expiry.date == "30/12/2020")
Call_mat2 <- Date_table_call %>% filter(Expiry.date == "30/12/2020")
...
#calculate the density for each price
x = 0:70
...
#plot RND
matplot(x,cbind(y.1), type="l", col=c("black", "blue"), xlab="Prices", ylab="Density", 
        lwd=c(2,2,2,2,2), lty = c(1,1,1,1,1), cex.axis = 1.25, cex.lab = 1.25, main= Selected_Date)

legend("topright", legend=c("21-08-2020"), col=c("black","blue","red"), 
