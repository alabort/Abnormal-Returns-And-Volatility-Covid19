...
Selected_Date <- "14/04/2020"
Selected_Date2 <- "14/04/20"

#import the prices of the underlying and filter the price of the wanted day
Price <-  read.csv("AIRCHS0.csv", header = TRUE, sep = ";", dec=".")
...
te1 <- as.numeric((as.Date("2020-09-29")-as.Date(Selected_Date, format = "%d/%m/%y"))/365) 
te2 <- as.numeric((as.Date("2020-12-30")-as.Date(Selected_Date, format = "%d/%m/%y"))/365) 
...
r1 <- as.numeric(as.vector(termStruct.spline.interpolate["2020-09-29", 1]))
r2 <- as.numeric(as.vector(termStruct.spline.interpolate["2020-12-30", 1]))
...
Main_table <- read.csv("ACH.csv", header = TRUE, sep = ",", dec=".")
#remember to change the date in the following line, the format should be X.dd.mm.yyyy
Date_table <- data.frame(Main_table$X14.04.2020, Main_table$Strike, Main_table$Expiry.Date, Main_table$X.NAME..y)
Date_table2 <- Date_table[complete.cases(Date_table), ]
Date_table_put <- Date_table2 %>% filter(Main_table.X.NAME..y== "P-AM")
Date_table_call <- Date_table2 %>% filter(Main_table.X.NAME..y== "C-AM")
...
Put_mat1 <- Date_table_put %>% filter(Expiry.date == "29/09/2020")
Call_mat1 <- Date_table_call %>% filter(Expiry.date == "29/09/2020")
...
legend("topright", legend=c("29-09-2020", "30-12-2020"), col=c("black","blue","red"), 
